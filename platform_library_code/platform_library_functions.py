...
#TODO Prevent well-plates from being added to the liquid handler in overlapping positions -- Error pathway
#TODO Check that well-plates are allowed to be added to the specified carrier -- Error pathway
...
def get_schema(type_tag):
    ...
    elif type_tag in ['solvents', 'reagents']:
        schema_template = {
            'type':'object',
            'properties':{
                'container_name': {'type':'string'},
...
def query_document(collection, search_term, search_field, mongo_database):
    ...
    relevant_collection = platform_database[collection]
    if search_field in ['location', 'campaign_name']:
        success_statements.append('The targeted search field is valid for multi-document returns, expect a dictionary of dictionaries')
        ...
        success_statements.append('The targeted search field is valid for single-document returns, expect a single dictionary')
        filecount = relevant_collection.count_documents({search_field: search_term})
        if filecount == 0 or filecount > 1:
            success_statements.append('Bad number of instances of "%s" in the database: %s instances' % (search_term, filecount))
            return ['Error', success_statements]
        else:
...
def move_database_document(db_doc_id, current_collection, new_collection, mongo_database):
    ...
    if current_collection not in valid_collections or new_collection not in valid_collections:
        ...
        return ['Error', success_statements]
    success_statements.append('The requested collections "%s->%s" are valid collections' % (current_collection, new_collection))
    if current_collection == new_collection:
        success_statements.append('The move "%s->%s" occurs in the same collection' % (current_collection, new_collection))
        return ['Error', success_statements]
    ...
    if filecount == 0 or filecount > 1:
        success_statements.append('Bad number of instances of "%s" in the %s collection: %s instances' % (db_doc_id, current_collection, filecount))
        return ['Error', success_statements]
    success_statements.append('Found %s instance of %s in the %s collection' % (filecount, db_doc_id, current_collection))
    ...
    if filecount > 0:
        success_statements.append('Bad number of instances of "%s" in the %s collection: %s instances' % (db_doc_id, new_collection, filecount))
        return ['Error', success_statements]
...
def update_database_document(previous_document, new_document, collection, mongo_database):
    ...
    if filecount == 0 or filecount > 1:
        success_statements.append('Bad number of instances of "%s" in the database: %s instances' % (new_document["_id"], filecount))    
        return ['Error', success_statements]
...
def query_platform_location(loc_tag, sub_location, mongo_database):
    ...
    for collection in valid_collections:
        ...
        file_cursor = current_db_collection.find({'location': {"$elemMatch":{"$in": [sub_location]}}})
        for file in file_cursor:
            if file['location'] == [loc_tag, sub_location]:
                gathered_documents['full_matches'][str(file['_id'])] = file
                gathered_documents['full_matches'][str(file['_id'])]['_id'] = str(gathered_documents['full_matches'][str(file['_id'])]['_id'])
            else:
                gathered_documents['partial_matches'][str(file['_id'])] = file
                gathered_documents['partial_matches'][str(file['_id'])]['_id'] = str(gathered_documents['partial_matches'][str(file['_id'])]['_id'])
...
def add_to_database(incoming_dict, collection, carriers_labware_dict, mongo_database):
    ...
    if collection in ['solvents', 'wellplates', 'reagents']:
        try:
            ...
            success_statements.append('Something is not correct in carrier_library for "%s"' % incoming_dict['labware_type'])
            print(carriers_labware_dict['labware'][labware_type])
            return ['Error', success_statements]
        ...
        if incoming_dict['contents'] == 'Empty' and collection == 'wellplates':
            ...
            if len(problem_wells) != 0:
                success_statements.append('Unexpected well names for "%s": %s' % (labware_type, ','.join(problem_wells)))
                return ['Error', success_statements]
        ...
        if filecount == 0:
            ...
            success_statements.append('Document already exists with container name "%s"' % incoming_dict['container_name'])
            return ['Error', success_statements]
        filecount = relevant_collection.count_documents({"container_name": incoming_dict['container_name']})
        ...
        if filecount == 0:
            ...
            success_statements.append('Document already exists for the consumable container_name "%s"' % incoming_dict['container_name'])
            return ['Error', success_statements]
        ...
        if filecount == 0:
            ...
            success_statements.append('Document "%s" already exists in the queue collection' % incoming_dict['queue_name'])
            return ['Error', success_statements]
    else:
...
def update_database_field(db_plate_name, update_field, old_value, new_value, mongo_database):
    ...
    if len(gathered_documents) != 1:
        success_statements.append('Bad number of instances of "%s" in the platform library: %s instances: %s' % (db_plate_name, 
                        len(gathered_documents), ' > '.join([str(file['_id']) for file in gathered_documents])))
        return ['Error', success_statements]
    else:
    ...
    if relevant_field != old_value:
        success_statements.append('Inconsistency between database value (%s) and sent old value (%s)' % (relevant_field, old_value))
        return ['Error', success_statements]
    ...
    success_statements.append('Updated the old value "%s" in document "%s" to new value "%s"' % (old_value, db_plate_name, new_value))
    return ['Success', success_statements]
...
def update_database(previous_collection, new_collection, collection, mongo_database):
    ...
    for new_doc_key in new_collection.keys():
        ...
        if document_changes == False:
            continue
        filecount = relevant_collection.count_documents({'_id':new_document["_id"]})
        if filecount == 0 or filecount > 1:
            return ['Error', 'Bad number of instances of "%s" in the database: %s instances' % (new_document["_id"], filecount)]
        relevant_collection.update_one({'_id':new_document["_id"]}, {"$set": without_keys(new_document, '_id')})
        success_statements.append('Updated document: %s' % str(new_document["_id"]))
