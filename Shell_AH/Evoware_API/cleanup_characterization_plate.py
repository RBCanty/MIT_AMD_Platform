        ...
        for well_to_clean in wells_to_clean.keys():
            ...
            if 'dilution_factor' in cleanup_methods.keys():
                ...
                if target_origin == 'reagent_tray':
                    return_statement, reagent_information = reagent_finder(full_library_mongo, target_reagent, additional_reagents_needed[well_to_clean])
                    if return_statement != 'Success':
                        problematic_wells.append(reagent_information)
                        continue
                    merge(additional_reagents_needed[well_to_clean], reagent_information)
                else:
                ...
                print(current_reagent_concentration, target_reagent_concentration)
        ...
        if additional_dilutions_needed:
            for target_destination in additional_dilutions_needed.keys():
                ...
                for seq_num, dilution_seq in enumerate(dilution_sequence):
                    ...
                    simplified_schedule.append(['MIX_LIHA', wellplate_home, tips, [200], wells, 10])
                    liha_tip_number += 1
...
if __name__ == "__main__":
    queuename = 'workflow_test_plate'
    prep_return = cleanup_characterization_plate(queuename, '3')
