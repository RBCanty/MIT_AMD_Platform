...
class SignalCheck(Peaks):
    ...
    def check_bounds(self, on_wells, on_iteration: int = 0):
        ...
        :param on_iteration: Which scan to use (None to attempt automatic lookup of most recent scan)
        :return: The wells which were out of range
        """
        if on_iteration is None:
            on_iteration = self.project.get_next_free_iteration() - 1
        bad_wells = dict()
        self.process_peaks(on_wells, on_iteration, sort_by='prominence', prominence=self.lower_bound / 100)
        data = self.project.get_property('peaks')
        data = data.get(str(on_iteration), dict())
