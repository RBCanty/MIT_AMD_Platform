...
def check_platereader(*_, **kwargs) -> mcs.RetObj:
    ...
    check_manager.tamer.subtract_wellplate_background(project)
    check_manager.tamer.subtract_background_by_solvent_volume(project, floor=0)
    bad_wells = check_manager.check_bounds(None)
    ...
    track_changes = 0
    try:
        doc, _, _ = dbi.query_document('SP', COL_WELLPLATES, DBG_CONTAINER_NAME,
                                       main_doc[Q_CONTAINERS][container][DBG_CONTAINER_NAME])
        edited_doc = deepcopy(doc)
        # Grab highest iteration number:
        new_iter = 0
        for k, v in bad_wells:
            e_doc_cont = edited_doc.get(DBG_CONTENTS, "Empty")
            if isinstance(e_doc_cont, str):
                e_doc_cont = dict()
            selection = e_doc_cont.get(k, dict()).get('additional_cleanup', dict())
            existing_iters = [(('0',) + re.search(r"(\d+)(?!.*\d)", _k).groups())[-1] for _k in selection.keys() if
                              "spark_cleanup" in _k]
            candidate = max([int(n) for n in existing_iters])
        ...
        for k, v in bad_wells:
            ...
            if isinstance(e_doc_cont, str):
                e_doc_cont = dict()
            selection = e_doc_cont.get(k, dict()).get('additional_cleanup', dict())
            if 'final' in selection.keys():
                continue

            ratio = target / v
            if ratio < 1:
                cleanup_arg = {'concentration_factor': ratio}
            else:
                cleanup_arg = {'dilution_factor': 1 / ratio}

            selection[cleanup_name] = cleanup_arg
            track_changes += 1
        if track_changes:
            dbi.update_document('SP', COL_WELLPLATES, doc, edited_doc)
        else:
    ...
    _success, _msg, _exc = aceso.request_sample_adjustment(q_name, step_num, cleanup_name, iter_num)
    if _success:
