...
def resource_request(queue_name, step, target_destination: str, is_new: bool, name_basis: str, plate_type=None,
                     generate=False, scan_barcode='no',
                     directly_specify: tuple = None):
    ...
    :param scan_barcode: 'yes'/'no' - Scans barcode after retrieval from LPX (can also be a bool)
    ...
    # Interpret scan_barcode as both a string or as a boolean
    if isinstance(scan_barcode, str):
        scan_barcode = scan_barcode.lower()
    if scan_barcode in ['yes', 'no']:
        pass
    elif scan_barcode:
        scan_barcode = 'yes'
    elif not scan_barcode:
        scan_barcode = 'no'
    else:
        return False, f"Invalid parameter 'scan_barcode' value ({scan_barcode})"
    step = int(step) - 1
...
def request_sample_adjustment(queue_name, step: str, cleanup_key: str, iter_num: int):
    """
    ...
    step_2 = build_step('Lh',
                        ...
                        time_est=60)
    step_3 = build_step('Lh',
                        operation='cleanup_characterization_plate',
                        details={'target_cleanup ': cleanup_key},
                        time_est=60)
    step_4 = build_step('Pr',
                        operation='Pr.prepare_to_send_or_receive',
                        details={QDET_PAIRED: 'yes',
                                 'schedule_time': 0},
                        time_est=5)
    step_5 = build_step('Lh',
                        operation='transfer_wellplate',
                        details={QDET_PAIRED: 'yes',
                                 'schedule_time': 0,
                                 'target_destination': 'spark'},
                        time_est=60)
    step_6 = queue_doc[Q_OPERATIONS_LIST][step]
    step_6.update({'completed': 'no',
                   'end_time': None,
                   'start_time': None,
                   'iter': iter_num - 1})

    queue_steps = [step_1, step_2, step_3, step_4, step_5, step_6]
